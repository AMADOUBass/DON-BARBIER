  ...
  function openEdit(stylist: Stylist) {
    ...
    setForm({
      ...
      availability: DEFAULT_AVAIL.map((d) => {
        const existing = stylist.availability.find(
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-brand-muted mt-1">
            {stylists.length} membres de l'équipe
          </p>
        </div>
        <button
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-brand-gold flex items-center gap-1">
                      ...
                      {stylist.yearsExp} ans exp.
                    </span>
                    <span
                      className={`w-2 h-2 rounded-full ${stylist.isActive ? "bg-green-400" : "bg-red-400"}`}
                    />
                    <span className="text-xs text-brand-muted">
                      {stylist.isActive ? "Actif" : "Inactif"}
                    </span>
                  </div>
                </div>
                <button
              ...
              {stylist.specialties.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {stylist.specialties.map((s) => (
                    <span
                      key={s}
                      className="text-xs bg-brand-gold/10 text-brand-gold border border-brand-gold/20 px-2 py-0.5 rounded-full">
                      {SPECIALTY_LABELS[s]}
                    </span>
                  ))}
                </div>
                    ...
                    <div
                      ...
                      className={`flex-1 text-center py-1 rounded text-xs font-medium ${
                        avail?.isActive
                          ...
                          : "bg-brand-charcoal text-brand-muted"
                      }`}>
                      {label[0]}
                    </div>
                  );
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Elvis 'Don' Berwa"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="contact@donbarbier.beauty"
                  />
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, yearsExp: e.target.value }))
                    }
                  />
                </div>
                <div>
      ...
      <AnimatePresence>
        {editStylist && (
          <motion.div
            ...
            className="fixed inset-0 bg-brand-black/80 flex items-center justify-center z-50 p-4"
            onClick={(e) =>
              e.target === e.currentTarget && setEditStylist(null)
            }>
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
                ...
                <div className="space-y-2">
                  ...
                  <UploadButton
                    ...
                    onClientUploadComplete={(res) => {
                      const file = res?.[0];
                      if (file) {
                        setForm((f) => ({ ...f, avatarUrl: file.url }));
              ...
              <div>
                ...
                <input
                  ...
                  value={form.yearsExp}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, yearsExp: e.target.value }))
                  }
                />
              </div>
                      ...
                      <label className="flex items-center gap-2 w-24 cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              availability: f.availability.map((a, j) =>
                                j === i
                        ...
                        />
                        <span
                          className={`text-sm ${d.enabled ? "text-brand-beige" : "text-brand-muted"}`}>
