...
const templates: Record<EmailTemplate, (data: Record<string, string | number>) => { subject: string; html: string }> = {
  appointment_confirmed: (d) => ({
    subject: "Don Barbier — Votre dépôt a été reçu ✓",
    html: `
  ...
  appointment_pending_interac: (d) => ({
    subject: "Don Barbier — En attente de votre virement Interac ⌛",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;border:1px solid #C9A84C33;">
        ...
        <div style="background:#1A1A1A;border:1px solid #C9A84C33;border-radius:8px;padding:20px;margin:24px 0;">
          ...
          <div style="margin-top:16px;padding-top:16px;border-top:1px solid #333;">
            <p style="margin:4px 0;"><strong>Option 1 — Par Courriel (Recommandé) :</strong></p>
            <p style="margin:4px 0;color:#F5EDD6;font-size:16px;font-weight:bold;">contact@donbarbier.beauty</p>
            
            <p style="margin:16px 0 4px 0;"><strong>Option 2 — Par Téléphone :</strong></p>
            <p style="margin:4px 0;color:#F5EDD6;font-size:16px;">(418) 929-9552</p>
          </div>
  ...
  appointment_accepted: (d) => ({
    subject: "Don Barbier — Rendez-vous confirmé ✓",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;">
        <h1 style="color:#C9A84C;">Don Barbier</h1>
  ...
  appointment_declined: (d) => ({
    subject: "Don Barbier — Mise à jour de votre rendez-vous",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;">
        <h1 style="color:#C9A84C;">Don Barbier</h1>
        ...
        <p>Veuillez nous contacter pour reprogrammer : <a href="tel:+14189299552" style="color:#C9A84C;">(418) 929-9552</a></p>
      </div>
    `,
  }),

  appointment_reminder: (d) => ({
    subject: `Don Barbier — Rappel : demain à ${d.appointmentTime}`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;">
        <h1 style="color:#C9A84C;">Don Barbier</h1>
  ...
  order_shipped: (d) => ({
    subject: "Don Barbier — Votre commande est en chemin !",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;">
        <h1 style="color:#C9A84C;">Don Barbier</h1>
  ...
  welcome: (d) => ({
    subject: "Bienvenue chez Don Barbier ✨",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;">
        <h1 style="color:#C9A84C;">Don Barbier</h1>
        <h2>Bienvenue, <strong>${d.name}</strong> !</h2>
        <p>Nous sommes ravis de vous accueillir au salon Don Barbier — votre destination premium pour les locs et coiffures afro.</p>
  ...
  reset_password: (d) => ({
    subject: "Réinitialisation de votre mot de passe — Don Barbier",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;border:1px solid #C9A84C33;">
        <h1 style="color:#C9A84C;font-size:24px;margin-bottom:8px;">Don Barbier</h1>
  ...
  google_signin_reminder: (d) => ({
    subject: "Connexion via Google — Don Barbier",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0A;color:#F5EDD6;padding:40px;border-radius:12px;border:1px solid #C9A84C33;">
        <h1 style="color:#C9A84C;font-size:24px;margin-bottom:8px;">Don Barbier</h1>
